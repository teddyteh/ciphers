...
export const encryptRailFenceCipher = (text: string, key: number) => {
...
export const decryptRailFenceCipher = (cipher: string, key: number) => {
  // create the matrix to cipher plain text
  // key = rows , text.length = columns
  const rail = new Array(key)
    .fill('')
